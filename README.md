...
functions/
└── index.js                # Cloud Functions: Gemini proxy + Jarvis controlled card/plan mutations
```
...
node scripts/export-trip-snapshot.mjs --trip tokyo-may-2026
```

在正式拆資料前，至少應先補：

- `export-trip-snapshot`：任何大改前先備份。
- `restore-trip-snapshot`：可明確恢復，而不是靠 session log 重建。
